...
BEGIN
    ----------------------------------------------------------------------
    -- Step 1: Update lookup fields in tblTraceabilityData for new records
    --         (Machine_Name, Model_Name, Part_No, Alarm_Message)
    ----------------------------------------------------------------------
    SET NOCOUNT ON;
    ...
    ----------------------------------------------------------------------
    -- Step 2: Update ProcessLineOverview Status for jobs where all machines are filled
    --         (Set Status to 'Complete' if Summary is OK, 'Warning' if NG)
    --         Only applies to jobs with all DATA_Code1-3 present
    ----------------------------------------------------------------------
    UPDATE plo
    SET Status = 
        CASE 
    ...
    ----------------------------------------------------------------------
    -- Step 3: Insert new machine process records into ProcessOverview
    --         (One row per machine per process event)
    ----------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------
    -- Step 4: Insert or update ProcessLineOverview for Machine_No = 1 (first machine for the line)
    --         If a row exists for the same Line_No, DATA_Code1-3, update it (rework), else insert new
    ----------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------
    -- Step 5: After rework/insert, always recalculate summary and status for the affected row
    --         (Summary/Status only set when all actual machines for the line are filled)
    ----------------------------------------------------------------------
    UPDATE plo
    SET 
        Summary = CASE
    ...
    ----------------------------------------------------------------------
    -- Step 6: Update ProcessLineOverview machine columns for Machine_No > 1
    --         (Set Machine_N column for the correct machine)
    ----------------------------------------------------------------------
    UPDATE plo
    SET 
...
AND ISNULL(i.DATA_Code2, '') = ISNULL(plo.DATA_Code2, '')
    ...
    ----------------------------------------------------------------------
    -- Step 7: (Legacy/Comment) -- Removed LogID-based summary update; all summary/status logic now uses (Line_No, DATA_Code1, DATA_Code2, DATA_Code3) as the job key
    ----------------------------------------------------------------------
END
GO
